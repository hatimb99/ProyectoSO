...
int Pon (ListaConectados *lista, char nombre[20], int socket) {
	//A̱ade nuevo conectados. Retorna 0 si ok y -1 si lalista ya
	// estaba llena y no ha podido a̱adir
	if (lista->num == 100)
		return -1;
	else {
	...
	while (terminar ==0)
	{
		...
		else if (codigo==4)
		{
			...
			else
			{
				...
				{
					sprintf(respuesta, "4/LogIn completado");
					encontrado=1;
				}
			}
		...
		if (codigo==4 && encontrado==1)
		{
			int res = Pon(&lista, nombre, sock_conn);
